 ...
 *  ABSENT when the agent reported no real usage (→ the UI dashes), never a fabricated 0. */
 ...
 *  (so the UI shows '—'), distinct from a real summed value (possibly small). */
...
export function aggregateRunMetrics(view: SessionView): RunMetrics {
  ...
  const latest = new Map<Role, { tok?: number; tools: number; ms: number }>()
  ...
  return { ...(totalTokens !== undefined ? { totalTokens } : {}), totalMs, perAgent }
}
