 ...
 * When ready, the user describes the app in the build composer below to start the agents.
 */
export function AkisChat({ api }: { api: ApiClient }) {
  const { t } = useI18n()
      ...
      <div className="flex-1 space-y-3 overflow-y-auto">
        {msgs.map((m, i) => m.role === 'assistant' ? (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="max-w-[80%] rounded-2xl rounded-tl-sm border border-white/10 bg-white/[0.04] px-4 py-2.5 text-slate-200">{m.content}</div>
          </div>
        ) : (
