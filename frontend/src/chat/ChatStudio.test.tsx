...
describe('Stüdyo nav → new chat (newChatSignal)', () => {
  ...
  it('a fresh chat shows an EMPTY Recent dropdown with the empty-state message (BUG A)', async () => {
    // The deep-linked build resolves AND /sessions/mine lists it (idea "an old build"), so the Recent
    // dropdown carries the previous conversation's build. After the Stüdyo-nav reset (newChat), the
    // dropdown must be EMPTY — showing the localized empty-state line, not the lingering old build.
    window.history.replaceState({}, '', '/?s=snav')
    const api = new ApiClient('', vi.fn(deepLinkFetch('snav', 200, { id: 'snav', status: 'done', version: 1 })))
    const fake = new FakeStream()
    render(wrap(<ChatStudio api={api} makeClient={() => fake as unknown as EventStreamClient} />))
    ...
    // BEFORE reset: the dropdown lists the prior build, then close it (Escape) so a later click re-opens.
    await userEvent.click(screen.getByRole('button', { name: /Recent/i }))
    expect(await screen.findByRole('menuitem', { name: /an old build/i })).toBeInTheDocument()
    await userEvent.keyboard('{Escape}')

    act(() => { requestNewChat() })
    await waitFor(() => expect(screen.queryByRole('button', { name: 'New build' })).toBeNull())

    // AFTER reset: open the dropdown again — no build rows, just the empty-state line.
    await userEvent.click(screen.getByRole('button', { name: /Recent/i }))
