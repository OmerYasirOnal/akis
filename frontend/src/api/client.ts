...
export interface AuthUser { id: string; name: string; email: string; provider?: 'github' | 'google' | 'password'; avatarUrl?: string; /** Derived server-side from the AKIS_ADMIN_EMAILS allowlist; present+true only for an admin (gates a future admin UI). */ isAdmin?: boolean }
