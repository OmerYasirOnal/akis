...
export class ApiClient {
   ...
   *  sessionId-less turns typed before this build existed. The server seeds them onto the new
   *  `session.chat` (a NON-gate column, via the generic store patch) so a CROSS-DEVICE reopen
   *  rehydrates the spec-shaping turns too. Empty/omitted ⇒ the body is byte-identical to before. */
