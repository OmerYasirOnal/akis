...
- Run the published image (no local build): each tagged release publishes a smoke-gated image to GHCR, so you can `docker run -p 3000:3000 ghcr.io/omeryasironal/akis-platform-mvp:latest` directly (Ollama-style) — **`linux/amd64` only; on an ARM host build locally instead** (`docker compose up --build`). See [`docs/SELF_HOSTING.md`](docs/SELF_HOSTING.md) for the full run + compose story.
