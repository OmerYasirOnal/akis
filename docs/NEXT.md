...
- **v1.0.0 full self-host release published (2026-07-09)** — PR #165 prepared the release docs/status, tag `v1.0.0` triggered the Release workflow, and the workflow built the self-host image, passed the keyless `/health` boot-smoke, pushed `ghcr.io/omeryasironal/akis-platform-mvp:v1.0.0` and `:latest`, and created the GitHub Release. PR #166 then stabilized a flaky deep-link frontend test; the post-fix `main` CI is green.
- **v0.1.0 published (2026-07-09)** — PR #163 merged the preview/chat/agent release train to `main`, then tag `v0.1.0` triggered the Release workflow. The workflow built the self-host image, passed the keyless `/health` boot-smoke, pushed `ghcr.io/omeryasironal/akis-platform-mvp:v0.1.0` and `:latest`, and created the GitHub Release.
