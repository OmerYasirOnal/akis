...
> **✅ Reconciled with shipped code (2026-06-03, refreshed after the #61–#71 deep-gap batch).** M0–M5 are merged (issues #5–#10 closed; see `git log`). Checkboxes below reflect what is actually in the tree, grounded in the file noted after each tick. The deep-gap batch since landed **real semantic embeddings (keyed)**, **BM25 persistence + a guarded `vector(N)` column**, and the **Scribe `retrieve_knowledge` tool-loop** — the NOTEs below have been updated. Items honestly **still unchecked / TODO**: real ANN-at-scale (the column/index exist but are not the ranking path), the Proto/Critic tool-loop, and sandbox isolation. For the canonical current state see `README.md`; for what's next see [`docs/NEXT.md`](NEXT.md).
...
- [x] event-driven ingest sources — `IngestionSink` subscribes per session for **conversation** (non-ephemeral `text` bus events only — `knowledge/IngestionSink.ts` `toIngest()`); **repo** + **upload** sources are real but call `RagService.ingest()` directly (`ingest/RepoSource.ts` on `confirmPush`, `ingest/UploadSource.ts` via `/api/knowledge`), not via the sink. HONEST SCOPE (ground-truth 2026-07-09): a distinct **agent-output** source (raw generated spec/code as grounding) does **not** exist — agents' spec/code reach retrieval only via narration text events. Building it is a tracked leftover (see the RAG-leftovers issue + `docs/plans/2026-07-09-productization-roadmap.md` Faz 3).
...
- [x] **Golden eval set (≥20 query→chunk pairs) + top-5 quality gate (F1-AC8)** — 26 golden query→expected-chunk pairs over a 20-doc fixture corpus (`backend/test/fixtures/golden-corpus.ts`, incl. 4 low-overlap synonym/paraphrase pairs), measured end-to-end through the REAL retrieval path (vector+BM25+RRF+rerank) with the offline deterministic `LocalEmbeddingProvider` and wired into the normal vitest suite (`backend/test/unit/retrieval-golden-eval.test.ts`). The offline embedder **clears the F1-AC8 ≥80% target** — measured **top-5 = 26/26 (100%)**; the gate is pinned at 0.85 so a real **end-to-end recall** regression trips it without flakiness. HONEST SCOPE: this guards end-to-end hit-rate, **not** fusion-specific discrimination — with the offline feature-hash embedder the vector and BM25 halves are correlated, so vector-only and full-hybrid score the same and no stable "fusion-teeth" pair is constructible offline; a fusion-specific guard would need the semantic API embedder (see the test header).
...
- **Exit:** ✅ round-trip green and gates untouched; ✅ the **quality gate** (golden-eval, top-5 ≥80% — measured 100%) now closes the last open M1 exit criterion.
