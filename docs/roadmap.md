...
- [x] **Golden eval set (≥20 query→chunk pairs) + top-5 quality gate (F1-AC8)** — 22 golden query→expected-chunk pairs over a 20-doc fixture corpus (`backend/test/fixtures/golden-corpus.ts`), measured through the REAL hybrid (vector+BM25+RRF) path with the offline deterministic `LocalEmbeddingProvider` and wired into the normal vitest suite (`backend/test/unit/retrieval-golden-eval.test.ts`). The offline embedder **clears the F1-AC8 ≥80% target** — measured **top-5 = 22/22 (100%)**; the gate is pinned at 0.85 (above the 0.80 floor, below the measured rate) so a real ranking regression trips it without flakiness.
