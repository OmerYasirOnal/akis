...
| `AKIS_USER_TOKEN_BUDGET` | `0` (unlimited) | **Per-user token quota** (the enforcement mechanism is shipped; the number is yours). An integer token budget per user per period; `0`/unset/NaN ⇒ unlimited. When set, a build is refused at start with `429 { code: 'QuotaExceeded', resetAt }` BEFORE any provider spend (fail-closed). The over-quota *product* policy (block vs BYO-key vs upsell) is the owner's. |
| `AKIS_USER_TOKEN_PERIOD` | `30d` | The rolling window for `AKIS_USER_TOKEN_BUDGET` — a named tier or `<n>d` / `<n>h` / a raw ms number. |
| `AKIS_PRO_TOKEN_BUDGET` | `0` (unlimited) | **Paid-tier (Pro) budget.** A Pro subscriber's token budget per period (same period as Free). `0`/unset ⇒ Pro is unlimited. Free uses `AKIS_USER_TOKEN_BUDGET`; the quota is resolved per the user's tier. |
| `AKIS_MAX_ACTIVE_RUNS` | `0` (unlimited) | **Per-user active-run cap.** Max simultaneously-running builds per owner; `0`/unset ⇒ unlimited. When set, `POST /sessions` + approve are refused at start with `429 { code: 'ConcurrencyLimited', activeRuns, limit }` while the owner already has that many pipeline-running sessions. Bounds simultaneous LOAD where the token quota bounds windowed SPEND; anonymous work is exempt (governed by `AKIS_REQUIRE_AUTH_FOR_BUILDS` + the anon quota). |
| `AKIS_RATE_LIMIT` | unset (off) | **Per-caller request-rate limiting.** Truthy (`1`/`true`/`on`) turns on a per-window flood guard on the expensive/write endpoints (build-start + approve, chat, external-write), keyed by owner (per-account) or IP (per-anon) — the primary anon-flood guard. Over the cap ⇒ `429 { code: 'RateLimited', retryAfter }`. Generous defaults (build 30/min, chat 60/min, external-write 30/min); tighten per bucket via `AKIS_RATE_LIMIT_<BUILD\|CHAT\|EXTWRITE>_MAX` / `_WINDOW_SEC`. Auth login/signup/forgot/reset/change-password are always rate-limited (per-IP) regardless. |
| `AKIS_ADMIN_EMAILS` | unset | **Human admin allowlist** (comma-separated emails). An allowlisted email is an admin **only when the account is OAuth-bound** (the email is provider-verified by Google/GitHub) — a **password account can never be an admin** even if its email is allowlisted, so nobody can escalate by registering an admin's email. An admin gets `isAdmin:true` on `/auth/me` and access to admin-only surfaces (the aggregate `GET /api/ops` + `GET /api/analytics`). **Unset ⇒ byte-identical:** no explicit admins; `/api/ops` + `/api/analytics` stay any-authenticated (single-operator dev/self-host). A MULTI-USER deployment SHOULD set this **and** have admins sign in via OAuth. Derived at read time — granting/revoking is an env change, never stuck in a session token. (Password-only admin needs an email-verification step, not yet shipped.) |
| `AKIS_OWNER_EMAIL` | unset | The single **owner** email — always treated as an admin (merged into `AKIS_ADMIN_EMAILS`). Also used as an OAuth allowlist (when set, only that provider-verified email may sign in via OAuth). |
| `STRIPE_SECRET_KEY` | unset (billing OFF) | **Paid tier — Stripe.** Your Stripe secret key (`sk_test_…` / `sk_live_…`). Unset ⇒ billing is dormant (`/billing/status` → `configured:false`, the Upgrade button hides). Sent only as a Bearer to api.stripe.com — never logged. |
| `STRIPE_PRICE_PRO` | unset | The recurring **Price ID** (`price_…`) the Pro checkout subscribes to — create a Product "AKIS Pro" + a recurring Price in your Stripe dashboard. Required (with the key) for billing to switch on. |
| `STRIPE_WEBHOOK_SECRET` | unset | The webhook **signing secret** (`whsec_…`). Add a Stripe webhook to `https://<your-host>/billing/webhook` (events: `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`) and paste its secret here. The webhook is rejected (400) without a valid signature — it's the only thing that grants Pro. |
