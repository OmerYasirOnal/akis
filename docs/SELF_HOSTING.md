...
| `AKIS_ADMIN_EMAILS` | unset | **Human admin allowlist** (comma-separated emails). An allowlisted (verified) email is an admin: `/auth/me` returns `isAdmin:true` and admin-only surfaces (e.g. the aggregate `GET /api/ops`) are restricted to admins. **Unset ⇒ byte-identical:** no explicit admins, and `/api/ops` stays any-authenticated (single-operator dev/self-host). A MULTI-USER deployment SHOULD set this to restrict operator surfaces. Derived at read time — granting/revoking is an env change, never stuck in a session token. |
