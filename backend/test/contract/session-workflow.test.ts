...
function build() {
  ...
  const saved = wfStore.save({ name: 'fast', agents: [{ role: 'scribe', model: { providerId: 'anthropic', modelId: 'claude-opus-4-8' } }], iterateBudget: 1 })
...
describe('per-session workflow selection (F2-AC9/AC10)', () => {
  it('binds a saved workflow to the session and routes its actions to that orchestrator', async () => {
    const { a, saved, calls } = build(); app = a
