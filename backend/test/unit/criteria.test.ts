...
describe('deriveChecks', () => {
  ...
  /**
   * USER-DATA literals are NOT body literals (the live "missing literal" failure class):
   * a client-rendered app injects typed/listed content via JS, so the SERVED body can never
   * contain it — asserting it is an impossible probe, not a real verification. Such criteria
   * fall through to the next derivable signal (path → render → skipped) instead.
   */
