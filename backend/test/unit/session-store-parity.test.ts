...
function paritySuite(name: string, make: () => SessionStore) {
  describe(`${name} parity`, () => {
    ...
    it('persists ADDITIVE externalWrites on the NORMAL update path and round-trips them via get() (durable on Pg too)', async () => {
      const store = make()
      await store.create(seed())
      const externalWrites: NonNullable<SessionState['externalWrites']> = [
        {
          id: 'w1',
          provider: 'atlassian',
          action: 'createJiraIssue',
          summary: 'Create Jira issue QR-12 in ENG',
          target: { projectKey: 'ENG' },
          payload: { summary: 'QR bug', description: 'Generated by AKIS.' },
          status: 'proposed',
          proposedAt: '2026-06-07T00:00:00.000Z',
        },
        {
          id: 'w2',
          provider: 'atlassian',
          action: 'createConfluencePage',
          summary: 'Create Confluence page in ENG',
          target: { spaceKey: 'ENG' },
          payload: { title: 'Release notes', body: '# Release notes' },
          status: 'executed',
          proposedAt: '2026-06-07T00:01:00.000Z',
          confirmedAt: '2026-06-07T00:02:00.000Z',
          result: 'https://example.atlassian.net/wiki/spaces/ENG/pages/1',
        },
      ]
      const next = await store.update('s1', { externalWrites }, 0)
      expect(next.version).toBe(1)
      expect(next.externalWrites).toEqual(externalWrites)
      const got = await store.get('s1')
