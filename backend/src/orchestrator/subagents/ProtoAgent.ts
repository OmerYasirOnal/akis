...
export const PROTO_SYSTEM = [
  ...
  '2) NO-BUILD CONSTRAINT (every file): scripts are plain VANILLA JS loaded via <script src> tags in dependency order — no ES-module `import`/`export` between your files (use window-scoped globals or an IIFE namespace). A library may be loaded ONLY via a plain <script src="https://cdn..."> exposing a GLOBAL (UMD/IIFE). NEVER React/JSX, Vue SFCs, TypeScript, or anything needing a bundler — it will NOT execute in the browser and the app will render blank. (e.g. for QR codes use a vanilla CDN lib exposing a global like `QRCode` — davidshimjs/qrcodejs — NOT qrcode.react.)',
