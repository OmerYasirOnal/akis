...
export const PROTO_SYSTEM = [
  ...
  '3) Persist client state with localStorage. Emit a real BACKEND only when the spec genuinely needs a server/API/shared data — then produce a "node-service" with EXACTLY this shape: a "server.js" using ONLY the Node standard library (node:http, node:fs — ZERO dependencies, no express, so install is instant and nothing can fail) + a package.json {"name":..., "main":"server.js"} (the preview runs `node .`). The server MUST listen on process.env.PORT (fallback 3000) on 127.0.0.1, serve the client files (index.html + assets via fs, correct content-types) at `/`, expose its JSON API under `/api/...`, and persist server data to a local "data.json" file (read on boot, write on change). The client calls the API with relative fetch("/api/...") paths.',
  "3b) FULL-STACK (ONLY when the spec explicitly needs USER ACCOUNTS/login or per-user relational data — for simple shared data without accounts use rule 3's data.json instead): same node-service shape, but persist in a REAL database via Node's BUILT-IN `node:sqlite` (`const { DatabaseSync } = require('node:sqlite')`, file `app.db`, CREATE TABLE IF NOT EXISTS on boot; built in since Node 22.13 — on an older runtime fall back to rule 3) — STILL zero npm dependencies; NEVER better-sqlite3/sqlite3 (native builds are blocked at install). Auth, all stdlib: hash passwords with node:crypto scrypt (per-user random salt; verify with timingSafeEqual AFTER an equal-length check, returning 401 on mismatch; NEVER store plaintext), sessions as an httpOnly cookie (`HttpOnly; SameSite=Strict; Path=/`) carrying a random token checked on every /api request; auth-required endpoints return 401 without it.",
  '4) Make it polished + responsive AND verify in your head that the core flow works end-to-end before returning — every <script src>/<link href> you reference MUST exist among the emitted files (a broken reference is a failed build). Do NOT include tests (the verifier writes those). Keep files focused.',
].join('\n')
