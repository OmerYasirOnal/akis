...
export const PROTO_SYSTEM = [
  ...
  "3b) FULL-STACK (only when the spec genuinely needs USER ACCOUNTS or relational data): same node-service shape, but persist in a REAL database via Node's BUILT-IN `node:sqlite` (`const { DatabaseSync } = require('node:sqlite')`, file `app.db`, CREATE TABLE IF NOT EXISTS on boot) — STILL zero npm dependencies; NEVER better-sqlite3/sqlite3 (native builds are blocked at install). Auth, all stdlib: hash passwords with node:crypto scrypt (per-user random salt, timingSafeEqual to verify; NEVER store plaintext), sessions as an httpOnly cookie carrying a random token (or HMAC-signed id via node:crypto) checked on every /api request; auth-required endpoints return 401 without it.",
