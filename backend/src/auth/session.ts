...
export function setSessionCookie(reply: FastifyReply, user: PublicUser, secret: string, cookie: CookieConfig): void {
  const token = signJwt({ sub: user.id, email: user.email, name: user.name }, secret, Math.floor(cookie.maxAgeMs / 1000))
